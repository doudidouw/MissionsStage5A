...
class digest_sasl_client_class {
	...
	Function Start ( &$client , &$message , &$interactions ) {
		...
		$this->credentials = array ( 'user' => '' , 'password' => '' , 'uri' => '' , 'method' => '' , 'session' => '' ) ;
		$defaults = array ( ) ;
	...
	Function Step ( &$client , $response , &$message , &$interactions ) {
		switch ( $this->state) {
			case SASL_DIGEST_STATE_RESPOND_CHALLENGE :
				...
				if (! IsSet ( $parameters [ $p = 'realm' ] ) && ! IsSet ( $parameters [ $p = 'nonce' ] )) {
				...
				$nc_value = '00000001' ;
				if (IsSet ( $parameters [ 'qop' ] ) && ! strcmp ( $algorithm, 'MD5-sess' ))
					$A1 = $this->H ( $this->credentials [ 'user' ] . ':' . $realm . ':' . $this->credentials [ 'password' ] ) . ':' . $nonce . ':' . $cnonce ; else
					$A1 = $this->credentials [ 'user' ] . ':' . $realm . ':' . $this->credentials [ 'password' ] ;
				$A2 = $this->credentials [ 'method' ] . ':' . $this->credentials [ 'uri' ] ;
				if (IsSet ( $parameters [ 'qop' ] ))
					$response = $this->KD ( $this->H ( $A1 ), $nonce . ':' . $nc_value . ':' . $cnonce . ':' . $qop . ':' . $this->H ( $A2 ) ) ; else
